...
class QuantumLoader:
    """
    Manages the 'Digital Lobotomy' and 'Quantum Loading' of Gemma-12B.
...
if __name__ == "__main__":
    import sys
    # Guessing path based on user context
    MODEL_PATH = r"H:\models\gemma-3-12B"
    
    print(f"Testing Quantum Loader with path: {MODEL_PATH}")
    if not os.path.exists(MODEL_PATH):
        print(f"❌ Path not found: {MODEL_PATH}")
        # Try finding it nearby
        parent = os.path.dirname(MODEL_PATH)
        if os.path.exists(parent):
            print(f"Available models in {parent}:")
            try:
    ...
    try:
        ...
        print("Testing Layer 0 Load...")
        layer0 = model.get_layer(0)
        layer0.forward("dummy") # Trigger load
        print("✅ Layer 0 Loaded.")
        
        layer0.reset_reality()
        print("✅ Layer 0 Unloaded.")
