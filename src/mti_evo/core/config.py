...
Use this instead of legacy mti_config.py.
"""
...
class MTIConfig:
    ...
    # Semantic Hardening (Phase F)
    anchor_file: str = None  # Path to anchors.json
    anchor_reinforcement_freq: int = 0  # Steps between reinforcements (0=disabled)
    pinned_seeds: set = field(default_factory=set) # Seeds that cannot be evicted
    anchor_saturation_threshold: float = 0.95 # Skip reinforcement if pre-resonance > this
    
    # --- Persistence (Phase J) ---
    persistence_dir: str = "cortex_data"
    mmap_capacity: int = 1024 * 1024 # 1 Million Neurons (~544MB)

    @property
    def seed(self) -> int:
        return self.random_seed
    
    # Plugins (V1: Explicit Hive Support)
    enable_hive: bool = False
    hive_config: Any = None # Holds HiveConfig from plugin when loaded
    
    enable_ghost: bool = False # Ghost Protocol (Code Indexer)
    
    # Persistence Injection
    persistence_manager: Any = None # Optional injection point
